...
const injectTo="code=code.replace(target,engine);const replayFrom=\"room.phase='question';room.questionIndex=0;room.answers.clear();room.questionStartedAt=Date.now();room.revealStartedAt=null;return json(res,200,{ok:true})\";const replayTo=\"for(const p of room.players.values())p.score=0;if(!room.usedQuestionTexts)room.usedQuestionTexts=new Set();for(const q of room.questions)room.usedQuestionTexts.add(q.q);const nextQuestions=pickGameQuestions(room.usedQuestionTexts);for(const q of nextQuestions)room.usedQuestionTexts.add(q.q);room.questions=nextQuestions;room.phase='question';room.questionIndex=0;room.answers.clear();room.questionStartedAt=Date.now();room.revealStartedAt=null;return json(res,200,{ok:true})\";if(!code.includes(replayFrom))throw new Error('Replay reset patch target not found');code=code.replace(replayFrom,replayTo);";
